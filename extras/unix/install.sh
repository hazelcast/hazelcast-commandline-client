...
setup () {
  detect_tmpdir
  commands="wget curl getopt sudo uname mkdir unzip tar"
  for cmd in $commands; do
      check_ok "$cmd"
  done
  detect_httpget
  for key in "${!state[@]}"; do
      log_info "$key: ${state[${key}]}"
  done
}
...
make_download_url () {
  ...
  state[download_url]="https://github.com/hazelcast/hazelcast-commandline-client/releases/download/$v/hazelcast-clc_${v}_${os}_${arch}.${ext}"
}
...
download_release () {
  ...
  local ext="${state[archive_ext]}"
  httpget "${state[download_url]}" > "$tmp/clc.${ext}"
}
