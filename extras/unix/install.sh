...
print_usage () {
    ...
    echo
    echo "Usage: $0 [--beta | --debug | --help | --version VERSION ]"
    echo
...
install_release () {
    ...
    local scripts
    set +e
    scripts="$(ls $base/scripts 2> /dev/null)"
    set -e
    if [[ "$scripts" != "" ]]; then
        # move builtin scripts
        local scripts_target="$CLC_HOME/scripts/builtin"
        mkdir -p "$scripts_target"
        for item in $scripts; do
            mv_path "$base/scripts/$item" "$scripts_target"
        done
        printf "Do not edit the scripts in this directory.\nThey may be overwritten by the installer.\n" > "$scripts_target/README.txt"
        log_info "Copied the scripts to $scripts_target"
    fi
...
detect_last_release () {
    if [[ "$state_download_version" != "" ]]; then
        return
    fi
...
process_flags () {
    ...
    while [[ "$flag" != "" ]]; do
        case "$flag" in
          ...
          --version*)
            shift
            state_download_version=$1
            if [[ "$state_download_version" == "" ]]; then
              # the version is missing
              bye "--version requires a version"
            fi
            ;;
    ...
    if [[ "$state_download_version" != "" ]]; then
        if [[ "$state_beta" == "yes" ]]; then
            bye "--beta and --version flags are mutually exclusive"
        fi
        local first_char=${state_download_version:0:1}
        if [[ "$first_char" != "v" ]]; then
            state_download_version="v$state_download_version"
        fi
    fi
}
